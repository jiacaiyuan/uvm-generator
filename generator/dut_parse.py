...
class DUT_PARSE(object):
    ...
    def get_parameters(self,local=False):
        ...
        for param in parameter_dict_list:
            ...
            elif param["Type"]=="localparam" and local==True:
                self.parameter[param["Name"]]=param["Value"]
    ...
    def get_module(self,module_name=""):
        ...
        module_list=format_get('module\s*(\S*)\s*[#|\(]',self.contents)
        if (module_name not in module_list) and len(module_list)>0:
            ERROR("set_module: "+"Can't Find Module "+module_name+" in "+str(module_list)+" Set "+module_list[0])
            self.module_name=module_list[0]
        else:
    ...
    def get_ports(self):
        ...
        module_content=self.contents[begin:end]
        ...
        port_dict_list=list2dict(['IO_Type','SIGNED','MSB','LSB','Name'],port_info)
        for port_info in port_dict_list:
            self.port_list.append(port_info["Name"])
            if port_info["IO_Type"]=="input":
                self.direction_list.append(format_bit(0)) #0=input 1=output 2=inout
            elif port_info["IO_Type"]=="output":
                self.direction_list.append(format_bit(1)) #0=input 1=output 2=inout
            else: #port_info["IO_Type"]=="inout":#0=input 1=output 2=inout
