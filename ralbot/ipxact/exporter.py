...
class IPXACTExporter:
    ...
    #---------------------------------------------------------------------------
    #def export(self, node, path): #jcyuan comment
    def export(self, node, path,filename):  #jcyuan
...
#       jcyuan add------------------------------  
        if not os.path.exists(path):
            os.makedirs(path)
        if path[-1]=="/":
            xml_file=path+str(filename)
        else:
            xml_file=path+"/"+str(filename)
        if ".xml" not  in xml_file:
            xml_file=xml_file+".xml"
        with open(xml_file,"w") as f:
            self.doc.writexml(f,addindent=self.xml_indent,newl=self.xml_newline,encoding="UTF-8")
