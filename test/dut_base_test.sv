//***** ***** ***** *****  *****  *****  *****  *****  *****  *****  ***** 
//Auto Generated by UVM-Generator
//Author: Jiacai Yuan
//E-mail: yuan861025184@163.com
//Contents:dut_base_test
//***** ***** ***** *****  *****  *****  *****  *****  *****  *****  ***** 
class dut_base_test extends uvm_test;
	`uvm_component_utils(dut_base_test)

	//Components
	dut_env env;

	//Constructor Function
	function new(string name="dut_base_test",uvm_component parent=null);
		super.new(name,parent);
	endfunction
	
	//Phase Methods
	extern virtual function void build_phase(uvm_phase phase);
	extern virtual function void connect_phase(uvm_phase phase);
	extern virtual task run_phase(uvm_phase phase);
	
	// Add user method declare here
	//e.g. task function
	// User method ends
endclass

function void dut_base_test::build_phase(uvm_phase phase);
	super.build_phase(phase);
	`uvm_info(get_name(),"Build Phase is Called",UVM_LOW)
	env=dut_env::type_id::create("env",this);
	
	// Add user build here

	// User build ends
endfunction

function void dut_base_test::connect_phase(uvm_phase phase);
	super.connect_phase(phase);
	`uvm_info(get_name(),"Connect Phase is Called",UVM_LOW)
	
	// Add user connect here

	// User connect ends
endfunction




task dut_base_test::run_phase(uvm_phase phase);
	super.run_phase(phase);
	`uvm_info(get_name(),"Run Phase is Called",UVM_LOW)
	phase.raise_objection(this);
	// Add user run here
	#100;
	//e.g. new sequence
	//e.g. start sequence
	// User run ends
	phase.drop_objection(this);
endtask